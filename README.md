...
| ☁️ **Scaleway Functions** | `handler.js` + `function.zip`，Serverless 按请求计费 | 轻量、免运维、自动扩缩 |
| ☁️ **Scaleway Containers** | Docker 镜像一键部署到 Containers | 常驻、支持长连接/SSE |

### ☁️ Scaleway 一键部署（推荐交互脚本）

> **最小兼容改动**：本分支在上游 `TiaraBasori/OpenCode2API v1.5.0` 上仅新增 `handler.js` + `src/config.js` 并微调 `src/proxy.js`/`index.js`/`entrypoint.sh`，实现同一代码在本地 / Docker / Scaleway Functions / Containers 四处运行。

```bash
# 克隆本分支
git clone https://github.com/<your-fork>/opencode2api-scaleway.git
cd opencode2api-scaleway

# 极度友好的交互式向导（配置 Zen Key、打包、部署）
bash scripts/scaleway-interactive.sh
# 或 ./scripts/scaleway.sh

# 非交互一键打包（CI 用）
bash scripts/pack-function.sh
# 生成 function.zip，handler=handler.handle，runtime=node22
```

**脚本功能一览**：

- 🎨 彩色 TUI 菜单、进度与校验
- 🔑 交互式配置 `OPENCODE_ZEN_API_KEY`（支持空=免费限流）、`API_KEY`（自动生成 `openssl rand -hex 16`）、`OPENCODE_SERVER_PASSWORD`
- 📦 一键 `zip -r function.zip handler.js src node_modules package.json`，自动 `npm ci --omit=dev`
- 🧪 本地仿真 `node handler.js` → `http://localhost:8080`，自动 `curl /health` 与 `/v1/models` 校验
- 🚀 `scw` CLI 一键创建 Namespace / Function / 上传 zip / 设环境变量 / 回显 URL
- 🐳 同时支持 `docker build` + `scw container` 部署到 Containers
- 🧹 清理、Dry-run、查看改动说明

**手动控制台部署**（无 scw 时）：

1. 打包：`bash scripts/pack-function.sh`
2. 打开 https://console.scaleway.com/functions/namespaces → Create Namespace → Create Function
3. Runtime `Node 22`, Handler `handler.handle`, 上传 `function.zip`
4. Environment variables：`API_KEY`（你的 Bearer）、`OPENCODE_ZEN_API_KEY`（留空走免费）、`OPENCODE_SERVER_PASSWORD`（可选）
5. Deploy → 复制 URL，测试：
```bash
curl https://<function-id>.functions.fnc.fr-par.scw.cloud/health
curl https://<function-id>.functions.fnc.fr-par.scw.cloud/v1/models \
  -H "Authorization: Bearer <API_KEY>"
curl https://<function-id>.functions.fnc.fr-par.scw.cloud/v1/chat/completions \
...
```bash
docker build -t rg.fr-par.scw.cloud/<ns>/opencode2api:latest .
docker push rg.fr-par.scw.cloud/<ns>/opencode2api:latest
# 控制台或 scw 创建 Container，端口选 10000，环境变量同上
```

> **免费额度提醒**：匿名 `Zen` 免费档为 IP 级 `~200 请求/日`（见 `opencode.ai/docs/zen`），Containers/Fuctions 出口 IP 共享更易触发限流，生产请配置 `OPENCODE_ZEN_API_KEY`。

<details><summary>改了哪里？（最小兼容 5 文件）</summary>

- **新增** `handler.js` — Scaleway `handler.handle`，复用 `createApp` + `serverless-http`，冷启动缓存
- **新增** `src/config.js` — 统一配置、`isScalewayFunctionEnv()` 探测、`PORT` 兼容 `8080`
- **新增** `scripts/pack-function.sh` / `scripts/scaleway-interactive.sh` — 完整 TUI
- **修改** `package.json` — `+serverless-http`，`scripts: dev:scaleway/pack:function`
- **修改** `src/proxy.js` — `import isScalewayFunctionEnv`、`ensureBackend` 在 Functions 下快速失败提示、`jail=/tmp`、修复重复 return
- **修改** `index.js` — 重构为 `buildConfig()`
- **修改** `entrypoint.sh` — 映射 `PORT→OPENCODE_PROXY_PORT`
</details>

📄 详见 [`docs/scaleway.md`](./docs/scaleway.md) 与 `scripts/scaleway-interactive.sh --help`
