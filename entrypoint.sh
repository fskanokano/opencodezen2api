...
# Scaleway Containers injects PORT (e.g. 8080) — map to PROXY_PORT if not explicitly set
if [[ -n "${PORT:-}" && -z "${OPENCODE_PROXY_PORT:-}" ]]; then
    export OPENCODE_PROXY_PORT="$PORT"
    echo "[Entrypoint] Mapped Scaleway PORT=$PORT -> OPENCODE_PROXY_PORT"
fi
# Scaleway Functions compatibility hint
if [[ -n "${SCW_FUNCTION_NAME:-}" || -n "${FUNCTION_NAME:-}" ]]; then
    echo "[Entrypoint] Detected Scaleway Functions env — container mode not recommended, use handler.js / function.zip"
fi
