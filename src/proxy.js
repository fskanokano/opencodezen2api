...
import { isScalewayFunctionEnv } from './config.js';
...
async function ensureBackend(config) {
    ...
    try {
        ...
        if (!MANAGE_BACKEND) {
            // Scaleway Functions: fail fast with actionable message instead of 120s wait
            const isSCW = isScalewayFunctionEnv();
            const maxRetries = isSCW ? 3 : STARTUP_WAIT_ITERATIONS;
            const interval = isSCW ? 500 : STARTUP_WAIT_INTERVAL_MS;
            ...
            if (isSCW) {
                const hint = `Backend ${OPENCODE_SERVER_URL} unreachable in serverless mode (MANAGE_BACKEND=false). ` +
                    `Fix: set OPENCODE_SERVER_URL to a reachable opencode host, or set OPENCODE_PROXY_MANAGE_BACKEND=true and bundle 'opencode-ai' binary, ` +
                    `or ensure OPENCODE_ZEN_API_KEY is set and backend is started elsewhere.`;
                console.error('[Proxy][SCW] ' + hint);
